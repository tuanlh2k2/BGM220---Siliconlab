...
static uint8_t set_timer = 5; // define timer = 5 ms.
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
	...
	case sl_bt_evt_gatt_server_attribute_value_id:
		...
		if (gattdb_device_control_characteristic == evt->data.evt_gatt_server_characteristic_status.characteristic) {
			set_timer = evt->data.evt_gatt_server_attribute_value.value.data[0];
			app_log("thoi gian gui du lieu la %d ms\n", set_timer);
			// stop timer.
			sl_simple_timer_stop(&app_periodic_timer);
			// reset timer
			sc = sl_simple_timer_start(&app_periodic_timer,
...
static void app_periodic_timer_cb(sl_simple_timer_t *timer, void *data)
{
	...
	if (status_of_device ==  DATA_TRANSMISSION) {
		...
		if (status_ring_buffer_pop != RING_BUFFER_ERROR || data_s[0] != 0 || data_s[2] != 0 || data_s[4] != 0) {
			sl_status_t sc;
