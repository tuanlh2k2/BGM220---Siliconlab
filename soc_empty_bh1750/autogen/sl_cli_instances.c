...
#if !ISEMPTY(SL_CLI_INST1_COMMAND_GROUP)
...
// Instance variables for inst1
#if defined(SL_CATALOG_KERNEL_PRESENT)
__ALIGNED(8) static uint8_t inst1_task_stack[(SL_CLI_INST1_TASK_STACK_SIZE * sizeof(void *)) & 0xFFFFFFF8u];
__ALIGNED(4) static uint8_t inst1_thread_cb[osThreadCbSize];
static osThreadId_t inst1_thread_id;
#endif
static sl_cli_t sl_cli_inst1;
sl_cli_handle_t sl_cli_inst1_handle = &sl_cli_inst1;
sl_cli_command_group_t sl_cli_inst1_command_group =
{
  { NULL },
  false,
  sl_cli_default_command_table
...
void sl_cli_instances_init(void)
{
  ...
  // ==== Initialize inst1 ====
  sl_cli_default_handle = sl_cli_inst1_handle;
  sl_cli_default_command_group  = &sl_cli_inst1_command_group;
  instance_parameters.task_name = "CLI instance inst1";
  instance_parameters.iostream_handle = SL_CLI_INST1_IOSTREAM_HANDLE;
#if ISEMPTY(SL_CLI_INST1_COMMAND_GROUP)
  sl_cli_default_command_group  = NULL;
#else
  sl_cli_default_command_group  = SL_CLI_INST1_COMMAND_GROUP;
  ...
  instance_parameters.default_command_group = sl_cli_default_command_group;
#if defined(SL_CATALOG_KERNEL_PRESENT)
  instance_parameters.thread_id = &inst1_thread_id;
  instance_parameters.thread_cb = inst1_thread_cb;
  instance_parameters.stack = inst1_task_stack;
  instance_parameters.stack_size = (SL_CLI_INST1_TASK_STACK_SIZE * sizeof(void *)) & 0xFFFFFFF8u;
  EFM_ASSERT(SL_CLI_INST1_TASK_PRIORITY < osPriorityISR);
  instance_parameters.prio = (osPriority_t)SL_CLI_INST1_TASK_PRIORITY;
  instance_parameters.start_delay_ms = SL_CLI_INST1_TASK_START_DELAY_MS;
  instance_parameters.loop_delay_ms = SL_CLI_INST1_TASK_LOOP_DELAY_MS;
#endif
  sl_cli_instance_init(sl_cli_inst1_handle, &instance_parameters);
...
bool sl_cli_instances_is_ok_to_sleep(void)
{
  ...
  if (sl_cli_is_ok_to_sleep(sl_cli_inst1_handle) == false) {
    return false;
  }
...
sl_iostream_t *previous = sl_iostream_get_default();
  ...
  // Handle inst1
  sl_iostream_set_default(sl_cli_inst1_handle->iostream_handle);
  sl_cli_tick_instance(sl_cli_inst1_handle);
