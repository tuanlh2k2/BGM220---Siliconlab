...
// chỉnh code ở đây => Thêm các sự kiện khác nữa vào.
typedef struct {
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    ...
    case sl_bt_evt_gatt_procedure_completed_id:
      ...
      if (conn_state == discover_services && conn_properties[table_index].thermometer_service_handle != SERVICE_HANDLE_INVALID) {
        // Discover (phát hiện) thermometer characteristic on the responder device
    ...
    // -------------------------------
    // This event is generated (được tạo ra) when a connection is dropped
