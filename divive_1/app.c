...
// device status.
typedef enum {
	DATA_TRANSMISSION,  // trạng thái gửi dữ liệu
	PENDING_TRANSMISSION // trạng thái chờ truyền dữ liệu.
}status_device;
...
SL_WEAK void app_process_action(void)
{
	if (flag_of_sensor == true) {
		...
		uint16_t tmp = (uint16_t) (data_light[0] << 8 | data_light[1]);
		if (sc1 & sc2 == 1) {
...
void sl_bt_on_event(sl_bt_msg_t *evt)
{
	...
	case sl_bt_evt_system_soft_timer_id:
		...
		if (evt->data.evt_system_soft_timer.handle == 1 && status_of_device == DATA_TRANSMISSION) {
...
static void app_periodic_timer_cb(sl_simple_timer_t *timer, void *data)
{
	...
	(void)timer;
	sl_status_t sc;
	uint8_t buf[6] = { integer_humi_value, decimal_humi_value, integer_temp_value, decimal_temp_value, data_light[0], data_light[1]};
	sc = sl_bt_gatt_server_send_indication(app_connection, gattdb_sensor_characteristic, sizeof(buf), buf);
	if (sc) {
